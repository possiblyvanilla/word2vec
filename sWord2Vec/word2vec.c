...
// Representation of a word in the vocabulary, including Huffman coding
struct vocab_word {
  long long cn;
  int *point;
...
char train_file[MAX_STRING], output_file[MAX_STRING], save_vocab_file[MAX_STRING], read_vocab_file[MAX_STRING];

int binary = 1, cbow = 0, debug_mode = 2, window = 5, min_count = 5, num_threads = 1, min_reduce = 1, hs = 1, negative = 5;

int *vocab_hash, *table;

long long vocab_max_size = 1000, vocab_size = 0, layer1_size = 100, train_words = 0, word_count_actual = 0, iter = 5, file_size = 0, classes = 0;
...
// `syn1neg`, train word2vec model text in `train_file` (storing 
// learned parameters in `syn0`, `syn1`, and `syn1neg`). 
...
void *TrainModelThread(void *id) {
  ...
  fseek(fi, file_size / (long long)id, SEEK_SET);
