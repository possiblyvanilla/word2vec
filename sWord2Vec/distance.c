...
int main(int argc, char **argv) {
  ...
  // read word vectors into array M
  for (b = 0; b < words; b++) {
    // read word
    a = 0;
    while (1) {
  ...
  while (1) {
    ...
    // tokenize (by spaces) input phrase in st1 into individual words
    // (store in st, an array of strings)
    cn = 0;
    b = 0;
    c = 0;
    while (1) {
    ...
    // increment cn so that it represents the number of words read
    cn++;
    // look up each word in phrase, storing the vocabulary
    // indices in array bi (respectively)
    for (a = 0; a < cn; a++) {
    ...
    // set vec to sum of (normalized) vectors of words in input sentence
